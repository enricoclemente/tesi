...
def main():
    ...
    if not os.path.isfile(args.resume):
        ...
        state_dict = dict()
        # print("cluster resnet")
        # dummy_input = torch.randn(10, 3, 32, 32)
        # cluster_resnet = ClusterResNet(num_classes=10, in_size=32)
        # torch.onnx.export(cluster_resnet, dummy_input, "cluster_resnet.onnx")
        # print(cluster_resnet)
        # print("moco encoder")
        # print(moco_model.encoder_q)
        # print("chekpoint keys")
        for key in checkpoint:
            if key.startswith("module.feature_module"):
                # print(key[22:])
                state_dict[key[22:]] = checkpoint[key]
        # print(moco_model.encoder_q.state_dict()['layer4.1.bn1.weight'])
        moco_model.encoder_q.load_state_dict(state_dict, strict=False)
        # print(moco_model.encoder_q.state_dict()['layer4.1.bn1.weight'])
...
def train(train_loader, train_original_images_loader, model, optimizer, epoch, train_writer, cfg, args):
    ...
    for b in range(total_batches):
        ...
        train_sub_iters = num_images // train_sub_batch_size
        ...
        for n in range(num_repeat):
            ...
            for i in range(train_sub_iters):
                ...
                end_idx = min((i + 1) * train_sub_batch_size, num_images)
