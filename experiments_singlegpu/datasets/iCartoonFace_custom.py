...
"""
    iCartoonFace dataset implementation for pytorch
    site: https://github.com/luxiangju-PersonAI/iCartoonFace
    paper: https://arxiv.org/pdf/1907.13394.pdf

    In this implementation of the dataset are taken only faces images, and 
    only validation ones that are enough (22k) for our purpose
...
class iCartoonFace(Dataset):
    ...
    def _read_metadata(self):
        ...
        classes_map = {"cartoon": 0}
        # create map of classes with number of images per classes { classname: # of images in class }
        classes_count = {"cartoon": 0}

        filtered_classes_count = {"cartoon": 0}
        total_filtered = 0

        # will be used to distribute in the same proportion each class into train and test
        classes_splitter = {"cartoon": 0}

        # will be used to distribute equally images among classes
        distributed_classes_count = {"cartoon": 0}

        class_name = 'cartoon'
        for img in os.listdir(os.path.join(self.root, 'personai_icartoonface_rectest/icartoonface_rectest')):
            total_images += 1
            classes_count[class_name] += 1
            filtered_classes_count[class_name] += 1

            W, H = Image.open(os.path.join(self.root, 'personai_icartoonface_rectest/icartoonface_rectest', img)).size
            ...
            if skip == False:
                ...
                if "train" in self.split:
                    if classes_splitter[class_name] < int(filtered_classes_count[class_name] * self.split_perc):
                        meta['split'] = "train"
                        meta['img_name'] = img
                        meta['img_folder'] = os.path.join('personai_icartoonface_rectest/icartoonface_rectest')
                        # put relative class index in targets since img_folder is equal to class name
                        meta['target'] = {'level1': 'cartoon', 'level2': class_name, 'level3': class_name,}
        ...
        # print(total_filtered)
        classes_map_hierarchical = {'level1': {'cartoon': 0}, 'level2': classes_map, 'level3': classes_map}
