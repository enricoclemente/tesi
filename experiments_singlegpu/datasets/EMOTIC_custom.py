        ...
        for s in ['train', 'val', 'test']:
            ...
            for i in range(len(split_annotations)):
                ...
                if split_annotations[i][1][0] != 'ade20k/images':
                    ...
                    img_path = os.path.join(split_annotations[i][1][0], split_annotations[i][0][0])
                
                    W, H = Image.open(os.path.join(self.root, 'cvpr_emotic', img_path)).size
...
""" 
    A new version for EMOTIC class dataset.
...
class EMOTIC_v2(EMOTIC):
    ...
    """
    def __init__(self, root: str, images_people_perc_metadata: str, split: Union[List[str], str] = "train", split_perc: float = 0.8, distribute_images = False,
                transform: Optional[Callable] = None, partition_perc: float = 1.0, aspect_ratio_threshold: float = None, dim_threshold: int = None, people_perc_threshold: int = 50):
        ...
        for s in ['train', 'val', 'test']:
            ...
            for i in range(len(split_annotations)):
                ...
                if split_annotations[i][1][0] != 'ade20k/images':
                    ...
                    img_path = os.path.join('EMOTIC/data/cvpr_emotic', img_path)
                    if img_path in img_people_perc and img_people_perc[img_path]['people_percentage'] < self.people_perc_threshold:
                        filtered_classes_count['nonselfie'] -= 1
                        total_images -= 1
                        total_filtered += 1
                    if self.aspect_ratio_threshold is not None and (W/H > self.aspect_ratio_threshold or W/H < 1/self.aspect_ratio_threshold):
