...
def main():
    ...
    save_folder = "/scratch/work/Tesi/LucaPiano/spice/code/SPICE/experiments_singlegpu/datasets/statistics/SocialProfilePictures/version_02"
    if not os.path.isdir(save_folder):
        os.mkdir(save_folder)

    print("Creating final dataset metadata")
    create_images_csv_version_02(images_csv_path="/scratch/work/Tesi/LucaPiano/spice/code/SPICE/experiments_singlegpu/datasets/SocialProfilePictures/data",
                                    save_folder=save_folder)

    print("\nCalculating statistics of SocialProfilePictures")
    dataset = SocialProfilePictures(root='/scratch/work/Tesi/LucaPiano/spice/code/SPICE/experiments_singlegpu/datasets', split=['train','test', 'validation'],
    ...
    images_examples_folder = os.path.join(save_folder, 'images_examples')
...
"""
def create_images_csv_version_01(total_images=60000, split_perc=0.8, aspect_ratio_threshold=2.33, dim_threshold=225, images_csv_path='./'):
...
"""
def create_images_csv_version_02(total_images=60000, split_perc=0.8, aspect_ratio_threshold=2.33, dim_threshold=225, images_csv_path='./', save_folder='./'):
    # indicates level0 classes and relative percentual presence in the final dataset
    level0_classes = {  'people': { 'perc': 0.6 },
                        'scenes': { 'perc': 0.2 },
                        'other': { 'perc': 0.2 }}
    ...
    # Take a partition of SIDD dataset == total_images * people perc
    EMOTIC_dataset_original = EMOTIC(root='/scratch/work/Tesi/LucaPiano/spice/code/SPICE/experiments_singlegpu/datasets/EMOTIC/data',
                split=["train", "test"], split_perc=split_perc)
    EMOTIC_dataset_filtered = EMOTIC(root='/scratch/work/Tesi/LucaPiano/spice/code/SPICE/experiments_singlegpu/datasets/EMOTIC/data',
                    split=["train", "test"], split_perc=split_perc, 
                    aspect_ratio_threshold=aspect_ratio_threshold, dim_threshold=dim_threshold)
    EMOTIC_partition_perc = min(level0_classes['people']['perc']/2 * total_images / len(EMOTIC_dataset_filtered), 1.0)
    EMOTIC_dataset_filtered_partition = EMOTIC(root='/scratch/work/Tesi/LucaPiano/spice/code/SPICE/experiments_singlegpu/datasets/EMOTIC/data',
